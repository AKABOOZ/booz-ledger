...
- 数据导入导出
  ...
  - 导入随手记 Excel 数据
- 多设备数据同步
  - WebDAV 双地址：局域网优先、外网自动兜底（两条地址必须指向同一目录）
  - “立即同步”：下载 NAS 最新完整备份、按稳定 ID 合并本机数据、再上传合并结果
  - 打开 App 时可自动执行同一套合并同步；不做后台常驻同步
  - 账号密码使用平台加密存储；NAS 保留最近 9 份历史备份
- 智能录入
  - 百度语音识别：语音转文本
  - 百度 OCR：图片文字识别
...
- 智能辅助录入
  ...
  - AI 理解账单并回填
- 数据同步（v1.3.4）
  - WebDAV 局域网/外网双地址自动选路
  - 全量 JSON 下载、按记录稳定 ID 合并、全量上传；删除使用 tombstone 防止旧设备数据复活
  - 同步页仅保留“立即同步”和“打开 App 时自动同步”，避免误触覆盖本地账本
  - 按 JSON 的 `exportedAt` 选择 NAS 最新备份，兼容旧/新备份文件名混存
  - 旧备份会先换算为“期初余额 + 流水”的结构，再参与合并，避免账户余额被重复累计；数据管理页提供一次性异常余额修复工具
- 备份治理
  - 最近一次需求已处理：
    - 本地备份只保留 `1` 份
    - NAS 仍保留 `9` 份
