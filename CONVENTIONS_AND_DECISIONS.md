...
### 决策 5.1：WebDAV 采用全量载荷的多设备合并同步（2026-09-04）

- 局域网与外网地址只是访问同一 NAS 目录的两条线路：局域网优先，失败后自动尝试外网。
- 日常“立即同步”严格执行：选路 → 下载 NAS 最新 JSON → 按稳定 ID 合并 → 上传合并后的完整 JSON。
- 每条账户、流水与自定义分类维护同步 ID 与修改时间；删除保留 tombstone，使旧设备的存量数据不会复活被删除的记录。
- NAS 备份按 JSON 内的 `exportedAt` 选择最新，不能将旧 `ledger_backup_*` 与新 `app-backup-*` 文件名混排后直接按名称判断。
- 同步页面只保留“立即同步”和“打开 App 时自动同步”；覆盖性 NAS 恢复不作为日常入口，以避免误覆盖本机数据。
- URL、账号与密码迁移到平台加密存储；非敏感同步状态仍存普通偏好设置。

已知边界：两台设备恰好同时完成下载后又同时上传时，后上传者可能暂时覆盖前者结果；任一设备再次同步即可重新合并。若后续需要严格避免该极端情况，应增加 WebDAV ETag/If-Match 或远端锁。
